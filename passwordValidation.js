...
var passwordCheckOld = function () {
    ...
    var stringPassword, arrayBadPasswords, stringOutput, stringPasswordLength;
    
    stringPassword = $("password").value;//gets user input of password
...
if (stringPassword.length < 8) {
   //console.log(stringPassword.length)
    stringOutput = "Your password must be at least 8 characters.";
}
else{

    for (var intCount = 0; intCount < arrayBadPasswords.length; intCount++) {
        if (stringPassword === arrayBadPasswords[intCount]) {//ifpw matches array
            stringOutput = "This password is not strong enough. Try something more complex.";
        }
    }
         if (stringOutput == "") {
            stringOutput = "Congrats, you made a great password!";
        }
    
}
  $("output").value = stringOutput;//statement that pushes stringOutput back to the DOM
